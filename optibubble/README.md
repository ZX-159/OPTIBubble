...
> Linux AppImage/deb/RPM via GitHub Actions).
...
stores by maintainers or community members — Homebrew (macOS) and `winget`/Scoop
(Windows) are the common upstream targets.
...
Apple Silicon), Linux deb, RPM and AppImage**. You trigger it
...
Linux RPM recipes — live in [`setup.md`](setup.md).**
...
├── requirements.txt
├── .github/workflows/main.yml  # manual release matrix (Actions → Run workflow; no tag)
