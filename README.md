...
[![Self-test](https://img.shields.io/badge/self--test-63%2F63%20green-22C55E)](#-development)
...
| 🎯 **Live edge-detection overlay** | The phone viewfinder draws a real-time framing quad from Sobel edge analysis (all on-device, ~4 ms/frame), with an *aligned — hold steady* state and optional **auto-capture** when the sheet is stable. |
...
stack and the < 3 s latency budget — **63 checks, expect all green**.
...
├── selftest.py              # 63-check end-to-end verification suite
...
python selftest.py              # 63-check end-to-end suite
