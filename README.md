...
[![Self-test](https://img.shields.io/badge/self--test-75%2F75%20green-22C55E)](#-development)
...
## 🎨 Design language — *Sightline*

The interface is built on **Sightline** — an instrument-grade dark UI with a
warm paper content plane, named after what the product does: putting a camera,
four corner anchors and a printed sheet on one line of sight.

| Token | Value | Role |
|---|---|---|
| Ink | `#07080B → #1B212A` | graphite chrome ladder — silent, hairline-separated |
| Chalk / graphite | `#EEF0F4 / #A6ADBA / #5A6270` | text ladder |
| Paper | `#F7F3EA` | the printed sheet — always the brightest object on screen |
| Iris | `#6D78F5` | **the machine**: selection, active state, primary action, data |
| Vermilion | `#FF5C38` | **the grading pen**: appears only where a human or the engine intervened — flags, evidence rings |
| Mint / amber / rose | verdicts | confident · ambiguous · double-marked (semantic only, never branding) |
...
instrument silkscreen; every comparable number uses tabular figures; a
9-px *registration-mark* crosshair precedes section labels, *corner brackets*
frame the viewfinder, sheet and evidence crops, and the *pen ring* — a
slightly rotated vermilion ellipse — marks any bubble a human has to look at.
One serif-italic moment per screen (Instrument Serif) where it matters.
Every control carries the full interactive-state matrix (idle · hover ·
focus-visible · active · disabled · loading · success · error). Fonts are
bundled locally — fully offline.
...
| 📈 **Psychometric analytics** | Per test: item error rates, discrimination (point-biserial) and **KR-20 reliability**, rendered live on the Results page. |
...
| 🎯 **Live anchor-lock overlay** | The phone viewfinder finds the four printed black squares in real time (on-device flood-fill + square filters, ~4 ms/frame) and locks corner brackets onto them — verified to sub-pixel accuracy — with an *aligned — hold steady* state and optional **auto-capture**. | The phone viewfinder draws a real-time framing quad from Sobel edge analysis (all on-device, ~4 ms/frame), with an *aligned — hold steady* state and optional **auto-capture** when the sheet is stable. |
...
stack and the < 3 s latency budget — **75 checks, expect all green**.
...
├── selftest.py              # 75-check end-to-end verification suite
...
python selftest.py              # 75-check end-to-end suite
